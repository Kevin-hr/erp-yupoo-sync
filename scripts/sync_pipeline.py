...
def get_cdp_browser_sync(default_cdp: str = "http://localhost:9222") -> Browser:
    ...
    async def _connect():
        ...
        _cdp_browser = await _pw_instance.chromium.connect_over_cdp(default_cdp, timeout=10000)
        ...
        try:
                        ...
                        for pg in pages:
                            try:
                                ...
                                if not pg_url or pg_url == "about:blank":
                                    continue
                                if target_url.split('/gallery/')[0] in pg_url or pg_url == target_url:
            ...
            if browser.contexts:
                ...
                try:
                    ...
                    logger.warning(f"[CDP] Failed to create new page in any context, closing browser: {e}")
                    try:
                        await browser.close()
                    except:
                        pass
...
class YupooExtractor:
    ...
    async def extract(self, page: Page) -> List[str]:
        ...
        if not photos:
            raise Exception(f"No photos found in API response for album {self.album_id}")
...
class MrShopLogin:
    ...
    def __init__(self, cookies_file: str = "logs/cookies.json"):
        ...
        logger.info(f"ERP credentials loaded - username: {self.email}, password length: {len(self.password)}")
    ...
    async def login(self, context: BrowserContext) -> bool:
        ...
        if self.cookies_file.exists():
            try:
                with open(self.cookies_file, 'r', encoding='utf-8') as f:
                    ...
                    logger.info(f"[Cookies] Loaded {len(cookies)} cookies from {self.cookies_file}")
                return True
            except Exception as e:
        ...
        try:
            ...
            await asyncio.sleep(5)  # 等待 Vue 路由跳转完成（SPA 异步渲染，networkidle 不够）
            # Check if login succeeded (should not be on login page anymore)
            if "login" in page.url:
                await page.screenshot(path=str(SCREENSHOT_DIR / "erp_login_failed.png"), timeout=60000)
                return False
...
class ImageUploader:
    ...
    async def upload(self, page: Page):
        ...
        logger.info(f"[Upload] Starting local-file upload for {len(self.urls)} images...")
        ...
        if not file_input:
            logger.warning("[Upload] All picture-card inputs failed, falling back to URL upload")
            await self._upload_by_url(page, self.urls)
            return
        ...
        for i in range(30):  # up to 30 seconds
            ...
            try:
                ...
                logger.info(f"[Upload] Progress check {i+1}/30: picture_card imgs={gallery_count}, success_list={success_count}")
                if gallery_count >= len(local_files) or success_count >= len(local_files):
                    logger.info(f"[Upload] Gallery has {gallery_count} images - upload confirmed!")
                    upload_done = True
                    break
                if i >= 5 and gallery_count == 0:
                    # After 5s, still 0 images - might be failing silently
                    logger.warning(f"[Upload] No images detected after {i+1}s, checking for errors...")
        ...
        logger.info(f"[Upload] Local file upload completed. ({len(local_files)} files submitted.)")
        ...
        async def handle_response(response):
            ...
            if 'photo.yupoo.com' in url and ('/small.' in url or '/big.' in url):
                try:
                    ...
                    # 从URL提取文件名
                    import re
                    match = re.search(r'/([^/]+)/small\.(jpg|jpeg)', url) or re.search(r'/([^/]+)/big\.(jpg|jpeg)', url)
                    if match:
                    ...
                    if str(local_path) not in local_paths:
                        ...
                        logger.info(f"[CDP-Download] {url} -> {local_path} ({len(body)} bytes)")
                except Exception as e:
                    logger.warning(f"[CDP-Download] Failed: {e}")

        # 注册临时监听器
        listener = lambda r: asyncio.create_task(handle_response(r)) if asyncio.current_task() else None
            ...
            try:
                ...
                if r.status_code == 200:
                    with open(local_path, 'wb') as f:
                        f.write(r.content)
                    local_paths.append(str(local_path))
                    logger.info(f"[Download] {url} -> {local_path} ({len(r.content)} bytes)")
                else:
                    logger.warning(f"[Download] HTTP {r.status_code} for {url}")
            except Exception as e:
                logger.warning(f"[Download] Failed to download {url}: {e}")

        logger.info(f"[Download] Completed: {len(local_paths)}/{len(urls_to_download)} images saved to {self.temp_dir}")
        return local_paths
        ...
        for attempt in range(3):
            try:
                ...
                if result == "clicked":
                    ...
                    logger.warning(f"[Verify] Save button not found (attempt {attempt+1})")
            except Exception as e:
...
class DescriptionEditor:
    ...
    async def format_description(self, page: Page):
        ...
        for frame in page.frames:
            if 'vue-tinymce' in frame.name or any('vue-tinymce' in str(f) for f in page.frames):
        ...
        if mce_frame:
            ...
            if result == True:
                ...
                logger.info("Successfully prepended formatted first line to product description. (成功在描述前插入格式化首行)")
            else:
                logger.warning("TinyMCE editor found but Name: block not found. (TinyMCE编辑器已找到但未发现Name:段落)")
        else:
            # Strategy 2: Fallback - try via iframe contentDocument access
            result = await page.evaluate(tiny_js, first_line_html)
            if result == True:
                logger.info("Successfully formatted (via contentDocument fallback). (通过contentDocument方式成功格式化)")
            elif result == 'prepended':
...
class SyncPipeline:
    ...
    async def run(self):
        ...
        async with async_playwright() as p:
            ...
            if self.use_cdp:
                cdp_browser = await p.chromium.connect_over_cdp("http://localhost:9222", timeout=10000)
                ...
                for pg in all_pages:
                    try:
                        ...
                        pass
                logger.info(f"[CDP] Tabs - Yupoo: {bool(yupoo_page)}, ERP: {bool(erp_page)}")
                # 创建新 Yupoo Tab 导航
            ...
            try:
                ...
                if cdp_mode:
                    ...
                    if "product/list" not in erp_page.url:
                        logger.warning(f"[CDP] Unexpected URL after navigation: {erp_page.url}, re-navigating...")
                        await erp_page.goto(erp_url, timeout=30000)
                        await erp_page.wait_for_load_state('networkidle', timeout=20000)
                        await asyncio.sleep(5)
                    logger.info(f"[CDP] ERP product list: {erp_page.url}")
                else:
                    already_logged_in = erp_page is not None and "login" not in erp_page.url and "mrshopplus" in erp_page.url
                    if already_logged_in:
                        logger.info(f"[CDP] Skipping login - using already logged-in ERP page: {erp_page.url}")
                        ...
                        if not login_success:
                            ...
                            raise Exception("ERP login failed. Please verify username and password in .env file.")
                        await erp_page.goto("https://www.mrshopplus.com/")
                        await erp_page.wait_for_load_state('networkidle', timeout=30000)
                        logger.info(f"After base navigation, current URL: {erp_page.url}")
                        await erp_page.goto(erp_url)
                        await erp_page.wait_for_load_state('networkidle', timeout=30000)
                        await asyncio.sleep(5)
                        logger.info(f"After product list navigation, current URL: {erp_page.url}")

                # Check if we are still on login page (检查是否仍然在登录页，说明登录失败)
                if erp_page is not None and "login" in erp_page.url:
                    logger.error("Still on login page after navigation - login failed or session expired")
                    # Take screenshot for debug
                    await erp_page.screenshot(path=str(SCREENSHOT_DIR / "login_failed.png"))
                    raise Exception("ERP login failed - still on login page after navigation. Please check credentials and manually login to get new cookies.")
                ...
                if not found:
                    ...
                    with open(LOG_DIR / "page_content.html", "w", encoding="utf-8") as f:
                        f.write(content[:20000])  # Save first 20KB
                    logger.error(f"No copy selector found. Page content saved to logs/page_content.html")
                    raise Exception(f"Cannot find copy button. Check login status and URL: {erp_page.url}")
                ...
                await safe_click(erp_page, ".operate-area .el-icon-document-copy", force=True)
                await erp_page.wait_for_load_state('networkidle', timeout=20000)
                ...
                try:
                    await erp_page.wait_for_selector("iframe[id^='vue-tinymce']", timeout=10000)
                    await asyncio.sleep(2)
                    logger.info("TinyMCE iframe detected, ready for description formatting.")
                except:
                    logger.warning("TinyMCE iframe not detected within timeout, proceeding anyway.")
                ...
                await erp_page.wait_for_load_state('networkidle', timeout=20000)
                await asyncio.sleep(8)  # Wait for full Vue mount including upload section
                logger.info(f"[Upload] Fresh navigation to copied product: {erp_page.url}")
                # Wait for upload section to mount
                for _ in range(15):
                    result = await erp_page.evaluate("() => { const ins = document.querySelectorAll('input[type=file]'); return Array.from(ins).some(i => i.multiple) ? 'found' : 'not_found'; }")
                    if result == 'found':
